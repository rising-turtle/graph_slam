 ...
 * Feb. 9, 2017 David Z
 *
 * convert the result of vro to ba or hybrid model
 *
 * this function is really a hopeless decesion, long story 
 *
 * */
...
{ 
  ...
  for(int i=0; i<gt_graph.mv_vro_res.size() && ros::ok(); i++)
  {
    ...
    if(model == "cut")
    {
      static ifstream inf("input_match.log");
      int id2, id1, num; 
      inf >> id2 >> id1 >> num; 
      
      if(id1 != pm->edge.id1 || id2 != pm->edge.id2)
      {
        ROS_ERROR("id1 = %d pm->edge.id1 = %d, not equal", id1, pm->edge.id1); 
        inf.close(); 
        return ; 
      }
...
bool changeMRByVRO(MatchingResult* pm)
{
   // VRO 
  ...
  (*g_ouf)<<pm->edge.id2<<" "<<pm->edge.id1<<" "; // <<match.size()<<endl;

  // Hybrid model 
  if(match.size() > 20)
  {
    (*g_ouf) << match.size() << endl; 
    // Let's do bundle 
    // update pm using ba 
  ...
  {
    // Let's do VRO 
    MatchingResult mr = nj->matchNodePairVRO(ni, Tji, &sr4k); 
    (*g_ouf)<<mr.inlier_matches.size() << endl;
    if(mr.succeed_match == false || mr.inlier_matches.size() <8) // VRO failed 
    {
      // cout <<"VRO failed for pm from "<<pm->edge.id1<<" to "<<pm->edge.id2<<endl; 
      if(match.size() >= 8) // VRO fails try BA instead
      {
        bool succeed = bundleAdjust(*pm, ni, nj, &sr4k); 
        if(!succeed) return false; 
        Eigen::Matrix4f I = Eigen::Matrix4f::Identity(); 
    ...
    }else
    {
      ...
      // *pm = mr; 
      // compute COV 
      pg->computeCovVRO(ni, nj, mr); 
      pm->final_trafo = mr.final_trafo;
      pm->edge.informationMatrix = mr.edge.informationMatrix; 
      cout <<"VRO succeed for pm from "<<pm->edge.id1 <<" to "<<pm->edge.id2<<endl;
    }
  }
  return true;
}
...
bool bundleAdjust(MatchingResult& mr, CCameraNodeBA* ni, CCameraNodeBA* nj, CamModel* pcam)
{
  ...
  map<int, int> match = nj->matchNodePairBA(ni, Tji, pcam);
