...
{
  CImuBase::getParam(); 
  ros::init(argc, argv, "ImuFactorsExample"); 
  ros::NodeHandle n; 
  ...
  // g.mp_node_values->insert(X(correction_count), prior_pose);
  // g.mp_node_values->insert(V(correction_count), prior_velocity);
  // g.mp_node_values->insert(B(correction_count), prior_imu_bias);  
  ...
  // g.mp_fac_graph->add(PriorFactor<Pose3>(X(correction_count), prior_pose, pose_noise_model));
  // g.mp_fac_graph->add(PriorFactor<Vector3>(V(correction_count), prior_velocity,velocity_noise_model));
  // g.mp_fac_graph->add(PriorFactor<imuBias::ConstantBias>(B(correction_count), prior_imu_bias,bias_noise_model));
  ...
  boost::shared_ptr<PreintegratedCombinedMeasurements::Params> p = CImuBase::getParam(); // PreintegratedCombinedMeasurements::Params::MakeSharedD(0.0);
  Vector3 g_tmp(0, 0 , 9.81); 
  // p->n_gravity = g_tmp; 
  ...
  while (file.good()) {
    ...
    if (type == 0) { // IMU measurement
      ...
      graph->add(BetweenFactor<imuBias::ConstantBias>(B(correction_count-1), 
       ...
       g.mp_fac_graph->add(BetweenFactor<imuBias::ConstantBias>(B(correction_count-1), 
                                                      B(correction_count  ), 
                                                      zero_bias, bias_noise_model));
#endif
      ...
      // g.mp_node_values->insert(X(correction_count), prop_state.pose());
      // g.mp_node_values->insert(V(correction_count), prop_state.v());
      // g.mp_node_values->insert(B(correction_count), prev_bias);

       LevenbergMarquardtOptimizer optimizer(*graph, initial_values);
       Values result = optimizer.optimize();
      // g.optimizeGraphBatch(); 
      // Values result = (*g.mp_node_values);
